...
public class StaticStudy {
    public static void main(String[] args) {
        ...
        snb2.sum(); //45
        /*
            객체마다 멤버필드와 공간이 각각 생기길 원하면 non-static (인스턴스 멤버필드)
            로 해야 한다. (각 객체마다 다른값을 저장 가능하게 하고 싶다면)
        ...
        // 매개변수만 이용하여 해결하는 메소드는 static method 로 만들면 좋다.
        // (즉 멤버필드 사용하지 않는 경우)

        Math.random(); //Math 클래스 안에 있는 random 이름의 static method
        // Math math = new Math(); // Meth() 앞에 private 해서 객체화를 불가했다.
        //math.random();
        // 이렇게 두 줄을 통해서 메소드를 호출할 필요가 없기 때문에 static 메소드가 사용이 편하다.
        // 그리고 Math 클래스는 객체화를 할 수 없다.
        // 이유는 기본 생성자 접근레벨을 private 세팅했기 때문  // p.229
    }
...
class StaticNumBox {
    ...
    static void sum(int n1, int n2) { //위에 StaticNumBox.sum(150,160); 이거 할때사용.
...
class NonStaticNumBox {
    ...
    void sum() {
        ...
        hello(); // 인스턴스 메소드에서 클래스 멤버메소드 호출 가능하다.
         // 왜 가능하냐? sum() 메소드가 호출이 된다는 것은  NonStaticNumBox 가 객체화가 되었다는
         // 것이고 static 은 이미 메모리에 올라가 있기 때문에 사용이 가능하다.
    }
